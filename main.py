...
def update():
    repo = g.get_repo("hello262/vanilla_updates")
    contents = repo.get_contents("update.svu")
    print(contents)
    ContentFile(path="update.svu")
...
async def verify(ctx):
    ...
    await ctx.send('Верификация ключа продукта...')
    if key == "lic_8989-9898-8989" or key == "lic_7474-4747-7474":
        status_verify = True
    else:
        status_verify = False
    await ctx.send('Данные получены, преобразование...')
    if status_verify == True:
        print("Расслабьтесь, ваша копия активирована!")
        await ctx.send("Данная копия активирована!")
    else:
        print("Упс, у вас подделка! После закрытия бота вы не сможете его запустить!")
        await ctx.send(f'Данная копия подделана! А это не есть хорошо! Поддельный ключ: {key}')
        await ctx.send("В целях безопасности данная копия была заблокирована.")
        os.rename("main.py", "8979797493747937493795349759037459873495790475473987490578")
        os.remove("8979797493747937493795349759037459873495790475473987490578")
...
lbl = Label(window, text="Версия:")    
lbl1 = Label(window, text=VER)
lbl2 = Label(window, text="Билд:")
lbl3 = Label(window, text=kVER)
btn = Button(window, text="Запуск", command=start)
btn_update = Button(window, text="Update", command=update)
